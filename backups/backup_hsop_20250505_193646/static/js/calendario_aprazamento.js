...
function renderizarCalendarioAprazamento() {
    ...
    // Buscar dados de aprazamento
    $.ajax({
        url: `/api/aprazamento/calendario/${window.internacaoId}`,
        method: 'GET',
        data: {
            hora_inicio: calendarioOptions.horaInicio,
            hora_fim: calendarioOptions.horaFim,
            intervalo: calendarioOptions.intervalo,
            mostrar_historico: calendarioOptions.mostrarHistorico ? 1 : 0
        },
        success: function(response) {
            if (response.success) {
                renderizarGradeHoraria(response.aprazamentos, response.dias);
            } else {
