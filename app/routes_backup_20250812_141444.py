...
@bp.route('/recepcionista/<int:funcionario_id>')
@login_required
def painel_recepcionista(funcionario_id):
    """
    Renderiza o painel da recepcionista.
    """
    try:
        ...
        if current_user.cargo.strip().lower() != 'recepcionista':
            flash('Acesso restrito a recepcionistas.', 'danger')
            return redirect(url_for('main.index'))
        
        # Verificar se o funcionario_id corresponde ao usuário logado
        if current_user.id != funcionario_id:
            flash('Você só pode acessar seu próprio painel.', 'danger')
            return redirect(url_for('main.index'))
        
        return render_template('recepcionista.html', funcionario=current_user)
        
    except Exception as e:
        logging.error(f"Erro ao acessar painel da recepcionista: {str(e)}")
        logging.error(traceback.format_exc())
