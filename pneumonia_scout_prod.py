"""
Script to scout for pneumonia patients in the PRODUCTION database
Period: 01/07/2024 to 31/11/2024
...
# Date range - ADJUSTED TO 2025 (real data from database: 27/05/2025 to 10/11/2025)
...
print("=" * 80)
print("SCOUTING FOR PNEUMONIA PATIENTS - PRODUCTION DATABASE")
print("=" * 80)
...
try:
    ...
    print(f"📊 Total internacoes in database: {total_internacoes}")
    ...
    if date_range[0] and date_range[1]:
        print(f"📅 Database date range: {date_range[0].strftime('%d/%m/%Y')} to {date_range[1].strftime('%d/%m/%Y')}")
    print()
    ...
    if len(results) == 0:
        ...
        print("\n🔍 Checking for ANY pneumonia-related CIDs in database...")

        # Check for any J18 variants
        j18_check = session.query(Internacao.cid_principal).filter(
            Internacao.cid_principal.ilike('%j18%')
        ).distinct().all()

        if j18_check:
            print(f"Found {len(j18_check)} J18 variants:")
            for (cid,) in j18_check:
        ...
        # Save to CSV
        output_file = f'pneumonia_patients_prod_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        if patient_data:
